 ...
 <div>
    <div class="card" style="margin-bottom: 10px">
      ...
      <el-input style="width: 260px; margin-right: 10px" v-model="data.name" placeholder="请输入姓名/学院查询"
                :prefix-icon="Search"/>
      <!-- <el-select v-model="data.col" clearable="" placeholder="请选择学院查询">
          <el-option v-for="citem in Colleges" :label="citem.name" :value="citem.id" :key="citem.id"></el-option>
        </el-select> -->
    ...
    <el-dialog width="35%" v-model="data.formVisible" title="学生信息" destroy-on-close>
      <el-form :model="data.form" :rules="rules" ref="formRef" label-width="100px" label-position="right" style="padding-right: 40px">
        <el-form-item label="学生账号" prop="username" v-if="!add">
          <el-input v-model="data.form.name" autocomplete="off" disabled/>
        </el-form-item>
        <el-form-item label="学生密码" prop="password" v-if="!add">
...
const search=()=>{
  ...
  request.get('/students/?search='+ data.name, {
     params: {
       page: data.pageNum,
       //pageSize: data.pageSize,
       username: data.username,
      ...
      if(add){
        ...
        if (res.status === 201) {
          ...
          let tempid=res.data.id
            request.post('http://127.0.0.1:8000/students/',{
              user:tempid,
              gender:data.form.gender,
              college:data.form.college.id,
            }).then(res=>{
...
const handleEdit = (row) => {
  ...
  if(data.form.college.id){}
  data.formVisible = true
}

const del = (id) => {
  ElMessageBox.confirm('删除数据后无法恢复，您确认删除吗？', '删除确认', { type: 'warning' }).then(res => {
    request.delete('/students/' + id + '/').then(res => {
      if (res.status === 204) {
        load()    // 重新获取数据
