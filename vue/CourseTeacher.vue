    ...
    <el-dialog width="35%" v-model="data.formVisible" title="课程信息">
      <el-form :model="data.form" label-width="100px" label-position="right" style="padding-right: 40px">
        ...
        <el-form-item label="任课老师">
          <el-input v-model="data.form.teacher_name" autocomplete="off" disabled/>
        </el-form-item>
...
let Colleges=[]
 ...
 request.get('/me/').then(res => {
    if (res.status === 200) {
      ...
      console.log('me id: '+me.id+' me username: '+me.name)
    } else {
...
const handleAdd = () => {
  data.form = {}
  data.form.teacher = me.id
  data.form.teacher_name=me.name
...
const save = () => {
  ...
  console.log(data.form.id)
       request.request({
         url: data.form.id ? '/courses/' + data.form.id + '/' : '/teachers/'+me.id+'/courses/',
...
const handleEdit = (row) => {
  ...
  if(data.form.teacher==me.id) data.formVisible = true
  else ElMessage.error('请勿修改其他老师的课程')
}

const del = (row) => {
  console.log(row)
  if(row.teacher==me.id)
  {
    ElMessageBox.confirm('删除数据后无法恢复，您确认删除吗？', '删除确认', { type: 'warning' }).then(res => {
      request.delete('/courses/' + row.id + '/').then(res => {
        if (res.status === 204) {
          load()    // 重新获取数据
          ElMessage.success("操作成功")
        } else {
  ...
  else ElMessage.error('请勿删除其他老师的课程')
}
</script>